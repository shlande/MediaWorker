...
func testJWTService(t *testing.T) (*jwt.JWTService, ed25519.PublicKey) {
	t.Helper()
	pub, priv, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatalf("generate control-plane key: %v", err)
	}
	svc := jwt.NewJWTService(priv, jwt.NewPeerIdSet(), jwt.NewRateLimiter(60*time.Minute),
		jwt.NewAuditLog(nil))
	return svc, pub
}

// signJWTForPeer signs a capability JWT for a peer using JWTService.
func signJWTForPeer(t *testing.T, svc *jwt.JWTService, peerID types.PeerId, nodePrivKey crypto.PrivKey) types.CapabilityJWT {
	t.Helper()
