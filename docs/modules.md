...
| **mwcli**（命令行客户端） | `cmd/mwcli/` | 开发用 CLI：`upload` 流式 multipart 上传至 ingest-worker（单遍 SHA-256）；`download` 内嵌完整边缘节点（复用 `internal/node/app`）接入 P2P 网络，经协议回源拉取 blob 并校验 SHA-256 | 无 HTTP 端口（CLI 运行） |
...
```
        ...
        CREDENTIAL_UPDATE│                      │ CONTENT_INGESTED
                 ┌───────▼───────────────────────┴────────────┐
                 │         edge-node 私有 P2P 网络             │
                  │  /edge/auth · /edge/blob/head · /edge/blob/get · /edge/l4/get
...
| 子模块 | 功能 |
|---|---|
| `app/` | 节点装配包（自 `cmd/edge-node` 抽取）：identity → peerstore → gater → libp2p host → JWT → DHT 发现 → 缓存 → ICP → 哈希环 → dataPlane/backhaul/l4fetch → pinstore → gossipsub → EdgeRouter 全链装配；`cmd/edge-node` 与 `cmd/mwcli` 共用同一装配代码 |
...
| `l4fetch/` | L4 流式回源协议 `/edge/l4/get/1.0.0`：服务端 handler（fetch 回调 = `BackhaulManager.HandleBlobL4`，失败 Reset 流）；客户端 `Fetcher`（筛 `L4Backhaul=true` 活跃 peer 轮询拉流，无候选返回 `ErrNoL4NodeAvailable`） |
...
### 2.5 mwcli 用法示例（`cmd/mwcli/`）

```bash
# 上传：流式 multipart POST 到 ingest-worker，单遍计算 SHA-256
mwcli upload -addr http://10.0.0.1:8080 -type image -file ./photo.jpg
# stdout:
#   content_id: <id>
#   blob_hash: sha256:<hex>   # image 类型 original blob 哈希，可直接用于 download

# 下载：内嵌边缘节点接入 P2P 网络，经协议回源拉取 blob 并校验 SHA-256
mwcli download -config configs/node-cli.local.yaml -blob sha256:<hex> -out ./photo.dl.jpg \
  [-wait-timeout 60s] [-req-timeout 120s]
```

退出码：`0` = 成功；`1` = 运行时/API 错误（含拉取失败）；`2` = 用法错误，或 download 在 `-wait-timeout` 内未发现可用 peer；`3` = 下载内容 SHA-256 校验失败。
