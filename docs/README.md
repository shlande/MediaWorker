...
| **L4 数据面 edge main 装配** | L4 节点 edge main 装配 `LocalDataPlane`（含 `HTTPLocationClient` + `func() string { return string(jwtClient.CurrentJWT()) }` 闭包），完整回源路径 FetchBlobLocal 可用 | **已接线**（commit 2a2f078、823851b）：节点装配已抽取至 `internal/node/app`；`access_layer.data_plane.enabled: true` 时构建完整数据面栈（`HTTPLocationClient` + `CurrentJWT` 闭包 + `AccountPool` + `LinkPool` + `LocalDataPlane`），注入 `BackhaulManager`（L4 模式），并注册 l4fetch 服务端 handler；非 L4 分支注入 `l4fetch.NewFetcher` 客户端。`cmd/edge-node` 变薄壳，复用同一装配代码。 | mwcli plan T3/T4；`internal/node/app/app.go` 步骤 17b/17c/T21。 |
| **节点间 L4 流式回退 + auth 交换发起** | 非 L4 节点经 libp2p stream 向 L4 节点流式回源（[`distribution/network.md §6.1`](distribution/network.md)）；连接建立时双方各自发起 `PresentAuth` 完成能力交换（[`distribution/network.md §3`](distribution/network.md)） | **已落地**（commit bd9fd41、2c38aed、2a2f078）：`internal/node/l4fetch` 落地 `/edge/l4/get/1.0.0`（线格式同 ICP：varint 前缀哈希 + 原始字节流；服务端失败 Reset 流）；`internal/node/app` 的 on-peer-connected 回调发起 `PresentAuth`（每 peer 60s 去抖、5s 超时），双向交换后 peerstore 获得 capabilities，哈希环不再恒空。此前 auth handler 存在但生产从未调用、`l4Fetcher` 为 nil 走 ICP 兜底，两项均由目标态转为实现态。 | mwcli plan T1/T2/T3；详见 [`protocols.md §2.1/§2.6`](protocols.md)。 |
