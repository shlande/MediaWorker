...
## [2026-07-20T04:10Z] Task: todo-42
- GET /v1/status in node adminapi: StatusDeps struct (plain values + 4 narrow interfaces) + Now() clock seam (todo-24 pattern) for deterministic uptime_sec. RegisterStatusRoutes — D1, no main.go edit.
- NetworkReporter returns PRE-AGGREGATED ConnCounts() (total,in,out), not []network.Conn: Go has no slice covariance and network.Conn is ~15 methods (v0.48.0: Stat() returns ConnStats embedding Stats — NOT Stats). Libp2pNetworkReporter(h.Network()) adapter lives in adminapi; test fake = struct embedding nil network.Conn + overriding only Stat().
- GraylistedCount scans LIVE scores (<= GraylistThreshold), NOT the sticky graylisted set — sets diverge (no un-mark path, scores recover); spec wants "当前低于阈值". Divergence locked by TestGraylistedCount_RecoveredPeerExcluded.
- Warm hit-rate + ttfb live in backhaul manager (spec-sanctioned): cacheReqs/cacheHits atomics in recordCacheRequest ALWAYS advance (prometheus increments stay metrics-gated); ttfbMs ring (cap 10k, obsMu, oldest-half eviction) mirrors todo-25 wins; recordTTFB on successful stream acquisition in l4.go singleflight (ttfb = stream-ready approximation, DataPlane interface can't observe first payload byte).
- prefix tier has NO read-serving path (HandlePrefixPull never mounted) → prefix rate恒 0 commented; relay_bytes_24h恒 0 (edge_relay_bytes_total has zero production callers).
